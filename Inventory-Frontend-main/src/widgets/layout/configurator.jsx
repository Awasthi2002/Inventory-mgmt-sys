...
export function Configurator() {
  ...
  const sidenavColors = {
    ...
    default: "from-[#7366F1] to-[#7366F1] border-gray-200", // Add your custom color
  };
  ...
  React.useEffect(() => {
    const fetchStars = async () => {
      try {
        ...
        if (data.stargazers_count) {
          setStars(formatNumber(data.stargazers_count, 1)); // Formatting if needed
        }
      } catch (error) {
    ...
    // Call fetchStars to get the star count
    fetchStars();

    // Set default sidenav color
    if (typeof setSidenavColor === 'function' && dispatch) {
      setSidenavColor(dispatch, "default");
    }
  }, []);

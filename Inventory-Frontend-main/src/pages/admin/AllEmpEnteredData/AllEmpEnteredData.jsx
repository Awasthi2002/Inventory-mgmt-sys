...
export function AllEmpEnteredData() {
...
// 2. Add handleDoubleClick function after existing handlers
...
// 3. Add handleDoubleClickSave function
const handleDoubleClickSave = useCallback((rowId, field) => {
    ...
    useEffect(() => {
        axios.get(`${config.apiurl}/offer/get-all-offers`)
            .then((response) => {
                const fetchedOffers = response.data || [];
                // Extract only _id and offerName, then sort by createdAt
                ...
                // Set the most recent offer as default if available
            ...
            .catch((error) => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Failed to fetch offers. Please try again.',
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                });
            });
    ...
    // const paymentOptions = ['credit_card', 'debit_card', 'paypal', 'bank_transfer', 'upi', 'wallet', 'cash'];
    ...
    Promise.all([
        ...
        .then(([response, imageResponse]) => {
            if (response.data.success && imageResponse.data.success) {
                ...
                // Construct the updated row, including paymentDetails, screenshot, and comment
                const updatedRow = {
                    ...finalData,
...
`;
        ...
        const responseData = await axios.get(`${config.apiurl}/offer/emp-entries-with-filter`, { params });
        setData(responseData.data.data || []);
    } catch (error) {
        console.error('Publish error:', error);
        let errorMessage = error.response?.data?.message || 'Failed to send data.';
    ...
    const columns = useMemo(
        () => [
            ...
            {
                accessorKey: 'employeeId.fullName',
    ...
    return (
        <div 
            style={{ display: 'flex', alignItems: 'center', maxWidth: 300, overflow: 'hidden', cursor: 'pointer' }}
...
[editingRow, doubleClickEditingCell, editData, validationErrors, EditableCell, EditableSelect, handleEdit, handleSave, handleCancel, handleDelete, handleDoubleClick, handleDoubleClickSave]
 );
...
export default AllEmpEnteredData;